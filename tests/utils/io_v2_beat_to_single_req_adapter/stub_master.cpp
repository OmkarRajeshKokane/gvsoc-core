...
private:
    ...
    // Request-path back-pressure, write flavour: a beat the adapter DENIED.
    // Ownership stayed with us — hold it unchanged and re-send it
    // (synchronously) from retry_handler. No further beats until it lands.
    vp::IoReq *held_wr_beat = nullptr;
    BurstState *held_wr_bs = nullptr;
    // Request-path back-pressure, read flavour: a read burst whose descriptor
    // the adapter DENIED (read-burst limit), held and re-sent from retry_handler.
...
void StubMaster::send_burst(BurstEntry *entry)
{
    ...
    // back-pressures the request (DENIED) — once the adapter's read-burst
    // limit is reached. On DENIED we hold the burst and re-send it
    // synchronously from retry_handler. It never replies inline (DONE).
    ...
    else if (st == vp::IO_REQ_DENIED)
    {
        ...
        printf("[%ld] %s REQHOLD name=%s write=0\n", now, this->logname.c_str(),
            entry->name.c_str());
        this->req_held = bs;
    }
    else
    {
        this->traces.assert(false,
            "adapter must return GRANTED or DENIED for a read burst (got %d)",
            (int)st);
    }
}
...
void StubMaster::send_write_beat(BurstState *bs)
{
    ...
    if (st == vp::IO_REQ_GRANTED)
    {
        // Beat consumed (the target frees it) — forget it.
        bs->wr_submitted++;
        if (last)
        {
    ...
    {
        // Per-beat back-pressure: ownership unchanged, hold the beat and
        // re-send it synchronously from retry_handler.
        this->traces.assert(this->held_wr_beat == nullptr,
            "a second write beat was denied while one is already held");
        printf("[%ld] %s REQHOLD name=%s write=1 beat=%d\n", now,
            this->logname.c_str(), e->name.c_str(), i);
        this->held_wr_beat = beat;
        this->held_wr_bs = bs;
    }
    else // IO_REQ_DONE
    {
...
{
    // Stalled on a denied write beat: retry_handler resumes the pipeline.
    if (this->held_wr_beat != nullptr)
    {
        return;
    }
    int64_t now = this->clock.get_cycles();
...
void StubMaster::issue_handler(vp::Block *__this, vp::ClockEvent *event)
{
    ...
    if (_this->held_wr_beat != nullptr)
    {
        return;   // stalled on request back-pressure; retry_handler resumes
    }
...
void StubMaster::retry_handler(vp::Block *__this, vp::IoRetryChannel)
{
    ...
    // ready — even when we have nothing held. Re-send whatever we hold now,
    // synchronously, per the io_v2 contract.

    // Held write beat (the adapter DENIED it — its downstream leg was denied).
    if (_this->held_wr_beat != nullptr)
    {
        vp::IoReq *beat = _this->held_wr_beat;
        BurstState *bs = _this->held_wr_bs;
        BurstEntry *e = bs->entry;
        bool last = beat->is_last;   // snapshot before re-send

        vp::IoReqStatus st = _this->out.req(beat);
        if (st == vp::IO_REQ_GRANTED)
        {
            _this->held_wr_beat = nullptr;
            _this->held_wr_bs = nullptr;
            bs->wr_submitted++;
            if (last)
            {
                _this->cur_wr = nullptr;
            }
            _this->arm_next();
        }
        else if (st == vp::IO_REQ_DENIED)
        ...
        {
            // Inline burst ack on the re-sent last beat (fast path).
    ...
    // Held read burst descriptor (read-burst limit).
