...
class StubTarget(gvsoc.systree.Component):
    ...
    Streams distinct allocator-backed read response beats one per cycle
    (several requests may be in flight, served FIFO) and checks/acks writes.
    Declaring ``IoV2Beat(beat_width)`` on its input makes the framework insert
    IoV2SingleReqToBeatAdapter when the bound master is ``IoV2SingleReq``.
    """
    def __init__(self, parent, name, beat_width=8, latency=0, error=False,
