...
class StubMaster(gvsoc.systree.Component):
    ...
    Declares signature ``IoV2Beat(beat_width)`` on its output so binding it to
    an ``IoV2Beat`` target of a *different* width makes the framework
    auto-insert IoV2BeatWidthAdapter.
    """
